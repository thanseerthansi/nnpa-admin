...
function Rss_News_List() {
  ...
  const rowNumber = (row) => rss_newses.indexOf(row) + 1;
  const columns = [
    {
      name: "#",
    ...
    {
      name: "HEADING",
      selector: (value) => <div style={{ wordWrap: "break-word", width: "350px",textAlign:'start' }}>{value.title?._text ?? value.title?._cdata ?? ""}</div>,
    },
    {
      name: "NEWS LINK",
      selector: (value) => <a href={value.link?._text ?? value.link?._cdata ?? ""} target="_blank" >
        <button className='btn btn-primary btn-xs'>Details </button></a>,
    },
    {
      name: "DESCRIPTION",
      selector: (value) => <div style={{ whiteSpace: "nowrap", width: "150px", maxHeight: "150px", overflow: "hidden", textOverflow: "ellipsis" }} dangerouslySetInnerHTML={{ __html: value.description?._text ?? value.description?._cdata ?? "" }} />,
    },
    {
      name: "PUB.DATE",
      selector: (value) => (handledate(value.pubDate?._text ?? value.pubDate?._cdata ?? ""))?.split('T')[0] ?? "",
    },
    {
      name: "ACTION",
      selector: (value) => <button className='btn btn-success btn-xs' onClick={() => Choose_Modal(value.title?._text ?? value.title?._cdata ?? "", <div dangerouslySetInnerHTML={{ __html: value.description?._text ?? value.description?._cdata ?? "" }} />, <div dangerouslySetInnerHTML={{ __html: value['content:encoded'] ? value['content:encoded']['_cdata'] : null }} />, value['dc:creator'] ? value['dc:creator']['_cdata'] : "", handledate((value.pubDate?._text ?? value.pubDate?._cdata ?? Date(""))))} >Save</button>,
    },
  ]
  const customStyles = {
    cells: {
