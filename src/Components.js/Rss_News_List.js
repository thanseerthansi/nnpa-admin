         ...
         <tbody>
            {
                rss_newses.length === 0 ? 
                <tr>
                    <td colSpan={6}> No Rss News Found</td>
                </tr>
                :
                <>
                {
                    rss_newses.map((value,key) => (
                        <tr key={key}>
                            ...
                            <td style={{textAlign:'left'}}><div style={{ whiteSpace:"nowrap",width:"150px",overflow:"hidden",textOverflow:"ellipsis"}} dangerouslySetInnerHTML={{ __html: value.description._text ?? value.description._cdata }} /> </td>
                            <td>{ value.pubDate._text ?? value.pubDate._cdata }</td>
                            <td><button className='btn btn-success btn-xs' onClick={()=>Choose_Modal(value.title._text ?? value.title._cdata ,<div dangerouslySetInnerHTML={{ __html: value.description._text ?? value.description._cdata }} /> )} >Save</button></td>
                        </tr>
                    ))
                }
                </>
